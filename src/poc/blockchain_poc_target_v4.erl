...
target_zone(RandState, Ledger) ->
    ...
    {ok, {Hex, HexRandState}} = choose_zone(RandState, HexList),
...
-spec target(
    ChallengerPubkeyBin :: libp2p_crypto:pubkey_bin(),
    ChallengerPrivkeyBin :: libp2p_crypto:pubkey_bin(),
    RandState :: rand:state(),
    Ledger :: blockchain_ledger_v1:ledger(),
    Vars :: map()
) -> {ok, {libp2p_crypto:pubkey_bin(), rand:state()}}.
target(ChallengerPubkeyBin, ChallengerPrivkeyBin, RandState, Ledger, Vars) ->
    %% Get all hexes once
    HexList = sorted_hex_list(Ledger),
    %% Initial zone to begin targeting into
    {ok, {InitHex, InitHexRandState}} = choose_zone(RandState, HexList),
    target_(
        ChallengerPubkeyBin,
        ChallengerPrivkeyBin,
        Ledger,
        Vars,
        HexList,
        InitHex,
...
target_(
    ...
    {RandVal, TargetRandState} = rand:uniform_s(ChallengerPrivkeyBin),
